	...
	std::vector<uint8_t> ebff_decoy(BinaryFixer& binary_fixer, const std::span<const uint8_t> text, const uint64_t image_base, const uint64_t text_base, std::vector<types::func_t>& funcs, std::vector<types::instruction_wrapper_t>& outside_fns_rip_jump_stubs) {
		...
		for (auto& fn : funcs) {
			...
			for (const auto& inst_wrapper : fn.decoded_insts_wrappers) {
				...
				uint64_t text_rel_inst_addr = inst_wrapper.inst.runtime_address - image_base - text_base;
		...
		std::vector<uint8_t> my_vector(0x1000, 0x90);
		new_text.insert(new_text.end(), my_vector.begin(), my_vector.end());
